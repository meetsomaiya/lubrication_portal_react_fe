          ...
          {filterBoxes[header] && (
            <div ref={filterBoxRef} className="filter-box">
              ...
              {Array.from(
                new Set(
                  filteredData
                    .map((row) => row[headerMapping[header]] ?? "") // Access the data using the mapping
                    .filter((value) =>
                      value
                        .toString()
                        .toLowerCase()
    ...
    {filteredData.map((row, index) => (
      <tr key={index}>
        {headers.map((header, idx) => (
          <td key={idx}>
            {/* Apply date formatting if the field is SAMPLE_COLLECTION_DATE */}
            {header === "SAMPLE_COLLECTION_DATE"
              ? formatDate(row[headerMapping[header]]) // Format date if it's SAMPLE_COLLECTION_DATE
