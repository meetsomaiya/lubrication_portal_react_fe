...
const fetchPlanStateWiseCount = async () => {
  ...
  const handleSelectChange1 = (event) => {
    setSelectedOption1(event.target.value);
       ...
       // Call the function to handle the Total card action
  ...
  const clearFilters = () => {
    ...
    useEffect(() => {
      if (totalCount > 0 && plannedCount > 0 && completedCount >= 0 && openCount >= 0) {
        const plannedPerc = (openCount / plannedCount) * 100;
        const openPerc = (completedCount / plannedCount) * 100;
        const completedPerc = (completedCount / totalCount) * 100;
    ...
    }, [totalCount, plannedCount, openCount, completedCount, outofGraceCount]);
