...
{/* <div className="filter-options-9963">
  {filterOptions9963
    .map((option) => 
      isNaN(option) || option === "" ? option : formatToTwoDecimals(option) // Apply formatting only to numbers
    )
    .sort((a, b) => String(a).toLowerCase().localeCompare(String(b).toLowerCase())) // Sort alphabetically
    .map((option, index) => (
