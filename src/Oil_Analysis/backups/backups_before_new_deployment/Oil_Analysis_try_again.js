...
const OilAnalysisTable = () => {
  ...
  const fetchDataForPendingTeco = async (event) => {
    ...
    if (year) {
      ...
      // Handle the click on the Order Type header
      const handleOrderTypeClick = async (orderType) => {
        if (orderType === 'FC_OIL_CHANGE ORDER') {
          if (!selectedYear) {
          ...
          const url = `http://localhost:224/api/fetch_fc_oil_chg_data?order_type=${encodeURIComponent(orderType)}&financial_year=${encodeURIComponent(selectedYear)}`;
          ...
          console.log('Order type is not FC_OIL_CHANGE ORDER');
        }
      };
  ...
  <div className="modal-7997">
    ...
    <thead>
      <tr>
        ...
        <th>Area Incharge Email</th>
        <th>Site Incharge Email</th>
        <th>Mapped State PMO</th>
      </tr>
    </thead>
    <tbody>
