...
const handleOrderTypeClick = async (orderType) => {
  ...
  const handleSegregatedFileDownload = async () => {
    // Log the retrieved cookie values
    console.log("Retrieved Cookies:");
    console.log("State:", state);
    ...
    const queryParams = new URLSearchParams({
      state,
      financialYear: selectedYear, // Assuming selectedYear is already managed in the component's state
    });
  
    // const apiUrl = `http://localhost:224/api/download_segregated_oil_analysis_file_user?${queryParams}`;
    const apiUrl = `http://localhost:3001/api/download_segregated_oil_analysis_file_user?${queryParams}`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        link.href = url;
        link.download = "segregated_file.xlsx"; // Adjust filename if needed
        link.click();
