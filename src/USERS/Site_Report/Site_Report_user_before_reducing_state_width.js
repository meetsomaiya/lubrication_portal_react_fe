...
function Site_Report_user() {
  ...
  const pathname = window.location.hash.replace(/^#/, '');
    ...
    useEffect(() => {
      ...
      // Split cookie values by comma and set them to state
      const stateList = decodedStateCookie ? decodedStateCookie.split(',') : [];
      const siteList = decodedSiteCookie ? decodedSiteCookie.split(',') : [];
      setAreas(decodedAreaCookie ? decodedAreaCookie.split(',') : []);
      setStates(stateList);
      setSites(siteList);
  
      // Set the first selected state as selectedSite1 if available
      if (stateList.length > 0) {
          setSelectedSite1(stateList[0]); // Using the first state as the selected site
      }
  }, []);
    ...
    return (
        ...
        <div className="sitereport-dropdown" id="state_dropdown">
