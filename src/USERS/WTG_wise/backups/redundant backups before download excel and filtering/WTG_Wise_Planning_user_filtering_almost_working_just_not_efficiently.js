...
useEffect(() => {
  ...
  const handleSelectChange1 = (event) => {
    ...
    // Function to handle checkbox changes and apply filtering
const handleCheckboxChange = (e, filterType) => {
  ...
  // Apply filtering after updating the selected filters
  applyFilters();
};


const applyFilters = () => {
  let filteredData = [...originalData]; // Keep a copy of the original data to filter

  if (selectedFunctionLoc991.length > 0) {
    filteredData = filteredData.filter(row => selectedFunctionLoc991.includes(row.FUNCT_LOC));
  }

  if (selectedPlant991.length > 0) {
    filteredData = filteredData.filter(row => selectedPlant991.includes(row.PLANT));
  }

  if (selectedOrderNo991.length > 0) {
    filteredData = filteredData.filter(row => selectedOrderNo991.includes(row.CRM_ORDERH));
  }

  setTotalData(filteredData); // Update the state with the filtered data
};
...
const fetchOpenStateWiseCount = async () => {
  ...
  const clearFilters = () => {
    // setSelectedFunctionLoc991([]);
    // setSelectedPlant991([]);
    // setSelectedOrderNo991([]);
    setTotalData(originalData); // Restore original data
  };
