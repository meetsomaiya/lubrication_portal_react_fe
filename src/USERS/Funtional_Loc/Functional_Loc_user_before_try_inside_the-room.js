...
//     handleSelectChangeSite3({ target: { value: functionalLocations[2]?.FUNC_LOC } });
//   }
...
const handleSelectChangeSite3 = async (event) => {
  ...
  if (funcLoc && funcLoc !== "Select") {
    ...
    // Automatically select first site option and second functional location option on page load
    useEffect(() => {
      // Automatically select the first available site (if available)
      if (sites.length > 0) {
          setSelectedSite2(sites[0]);
      }

      // Automatically select the second functional location option after 1 second
      const timeoutId = setTimeout(() => {
          if (functionalLocations.length > 1) {
              setSelectedSite3(functionalLocations[1].Functional_Location);
          }
      }, 1000);

      // Cleanup the timeout on component unmount
      return () => clearTimeout(timeoutId);
  }, [sites, functionalLocations]);
