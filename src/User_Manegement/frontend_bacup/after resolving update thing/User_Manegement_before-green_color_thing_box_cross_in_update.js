...
function UpdateModal({ user, onClose, onSave }) {
    ...
    useEffect(() => {
        const fetchStates = async () => {
            try {
                const response = await fetch(`${BASE_URL}/api/get_states`);
                const data = await response.json();
                setStatesData(data.map((item) => item.name));
            } catch (error) {
                console.error('Error fetching states:', error);
            }
        };
    ...
    const handleStateChange = async (selectedState) => {
        ...
        try {
            ...
            const data = await response.json();
            setAreasData(data.map((item) => item.area));
        } catch (error) {
            console.error('Error fetching areas:', error);
        }
    };
    ...
    const handleAreaChange = async (selectedArea) => {
        ...
        try {
            ...
            const data = await response.json();
            setSitesData(data.map((item) => item.site)); // Extracting "site" values
        } catch (error) {
            console.error('Error fetching sites:', error);
        }
    };
                ...
                <form>
                    ...
                    <label>
                        <select value={state} onChange={(e) => {
                            ...
                            handleStateChange(e.target.value); // Trigger area fetch
                        }}>
                            <option value="">Select State</option>
                            {statesData.map((stateOption, index) => (
                    ...
                    <label>
                        <select value={area} onChange={(e) => {
                            ...
                            handleAreaChange(e.target.value); // Trigger site fetch
                        }}>
                            <option value="">Select Area</option>
                            {areasData.map((areaOption, index) => (
                    ...
                    <label>
                        <select value={site} onChange={(e) => setSite(e.target.value)}>
                            <option value="">Select Site</option>
                            {sitesData.map((siteOption, index) => (
